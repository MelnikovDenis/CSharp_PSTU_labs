...
class Graph{
      ...
      public bool[,] AdjacencyMatrix {get; private set;} //матрица смежности
      public char[] Names { get; private set; } //имена вершин
      //кол-во вершин графа
      public int Count {get{
            return AdjacencyMatrix.GetLength(0);
      }}
      public Graph(bool[,] AdjacencyMatrix, char[] Names){
            if(AdjacencyMatrix.GetLength(0) == AdjacencyMatrix.GetLength(1) && AdjacencyMatrix.GetLength(0) == Names.Length){
                  this.AdjacencyMatrix = AdjacencyMatrix;
                  this.Names = Names;
            }                  
            else
                  throw new ArgumentException("Длина измерений матрицы смежности длжна быть одинаковой и равняться длине массива имён");
      }
      ...
      //транспонирование
      private static T[,] Transpose<T>(T[,] sourceMatrix){
            T[,] resultMatrix = new T[sourceMatrix.GetLength(1), sourceMatrix.GetLength(0)];
            for(int i = 0; i < resultMatrix.GetLength(0); ++i){
                  for(int j = 0; j < resultMatrix.GetLength(1); ++j){
      ...
      public List<List<int>> GetTieredParallelForm(){
            var adjacencyMatrixCopy = (bool[,])AdjacencyMatrix.Clone(); //клонируем матрицу смежности, чтобы столбцы занулялись не в основной матрице
      ...
      public bool[,] GetStronglyConnectedComponentMatrix(){
            ...
            foreach(var exp in GetBooleanMatrixExp()){
                  strConnCompMatrix = Graph.BooleanMatrixOR(strConnCompMatrix, exp);
            } //получение суммы E^0 + E^1 + ... + E^Count = R
            strConnCompMatrix = Graph.BooleanMatrixAND(strConnCompMatrix, Graph.Transpose<bool>(strConnCompMatrix)); //получение R & Transpose(R) = S
      ...
      private IEnumerable<bool[,]> GetBooleanMatrixExp(){
            var expMatrix = AdjacencyMatrix;
            for(int i = 0; i < Count; ++i){
                  yield return expMatrix;
                  expMatrix = BooleanMultiplyByMatrix(expMatrix, AdjacencyMatrix);
            }                  
      }
}